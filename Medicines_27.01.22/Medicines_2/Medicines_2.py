...
class Medicine:
    ...
    def get_numbers(self):
        ...
        medicine_numbers.append(re.findall(r'(\d+[.,]\d+|\d+)', self.NM_CLI))
        medicine_numbers.append(re.findall(r'(\d+[.,]\d+|\d+)', self.NM_FULL))
        medicine_numbers.append(re.findall(r'(\d+[.,]\d+|\d+)', self.PROD))
        medicine_numbers.append(re.findall(r'(\d+[.,]\d+|\d+)', self.GROUP_NM_RUS))
        #print(medicine_numbers)
        out = []
        for element in medicine_numbers:
            preout = []
            for number in element:
                number = float(re.sub(r',', r'.', number))
                print(number)
                if number != 0:
                    if number < 1:
                        while number < 1:
                    ...
                    if number > 1:
                        while number % 10 == 0:
                            number = number / 10
                    preout.append(str(float(number)))
                #if number == 0:
                    #preout.append([])
            out.append(preout)
        print(out, '!')
...
for element in tqdm(result):
    with open('NM+PROD_1.txt', "a", encoding="utf-8") as file:
        file.write(str(float(element[0])) + '\t' + element[1].CD_DT + '\t' +
